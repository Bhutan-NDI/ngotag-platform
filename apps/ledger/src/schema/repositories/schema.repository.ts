...
export class SchemaRepository {
  ...
  async getSchemaByOrgSchemaUrl(schemaUrl: string, orgId: string): Promise<schema> {
    try {
      ...
      this.logger.error(`Error in getting schema by org id and schema ledger id: ${error}`);
      throw error;
    }
  }

  async updateW3CSchemaLedgerDetails(updateSchema: IUpdateW3CSchemaLedger): Promise<schema> {
    try {
      ...
      this.logger.error(`Error in updating W3C schema ledger details: ${error.message} `);
      throw error;
    }
  }
