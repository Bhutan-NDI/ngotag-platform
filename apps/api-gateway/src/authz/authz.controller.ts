...
export class AuthzController {
  ...
  async addUserDetailsUserNameBased(
    @Body() userInfo: AddUserDetailsUsernameBasedDto,
    @Res() res: Response
  ): Promise<Response> {
  ...
  async usernameLogin(@Body() loginUserDto: LoginUserNameDto, @Res() res: Response): Promise<Response> {
    if (loginUserDto.username) {
      const userData = await this.authzService.usernameLogin(
        loginUserDto.username,
        loginUserDto.password,
        loginUserDto.isPasskey
      );
